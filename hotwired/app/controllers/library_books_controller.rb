class LibraryBooksController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @library_book.update(library_book_params)
        format.html { redirect_to library_book_url(@library_book), notice: "Library book was successfully updated." }
  ...
  private
    ...
    def library_book_params
      params.require(:library_book).permit(:title, :status, :description, :status_dat, :internal_info)
    end
end
